...
class RequestPermissionsUtil(private val context: Context) {
    ...
    public val REQUEST_LOCATION = 1

    /** 위치 권한 SDK 버전 29 이상**/
    @RequiresApi(Build.VERSION_CODES.Q)
    public val permissionsLocationUpApi29Impl = arrayOf(
    ...
    public val permissionsLocationDownApi29Impl = arrayOf(
        android.Manifest.permission.ACCESS_FINE_LOCATION,
        android.Manifest.permission.ACCESS_COARSE_LOCATION
    )

    /** 위치정보 권한 요청**/
    fun requestLocation() {//manifest에 권한 설정 되어있으면 유저에게 권한 요청
